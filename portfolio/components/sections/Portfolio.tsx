    ...
    <section id="portfolio" className="px-6 sm:px-12 lg:px-24 py-24 min-h-screen">
      <div data-aos="fade-up">
        <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-secondaryBg border border-black/10 dark:border-white/10 mb-10">
          <span className="text-xs font-bold text-textSecondary uppercase tracking-[0.2em] pt-0.5">Portfolio</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl transition-all duration-1000 md:grid-flow-row-dense">
        {projects.map((project, i) => {
          if (project.type === "featured") {
            ...
            return (
              <div key={project.title} data-aos="fade-up" data-aos-delay={100 * i} className="md:col-span-1 h-full">
                <a href={project.link} className="group flex flex-col h-full glass-panel rounded-3xl overflow-hidden transition-all duration-500 hover:border-[#D946EF]/30 bg-secondaryBg/50">
                  <div className="aspect-video min-h-[250px] overflow-hidden relative">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={project.image} alt={project.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                    <ExternalLink className="absolute top-6 right-6 w-6 h-6 text-foreground opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  </div>
                  <div className="p-8 flex flex-col flex-1 bg-background/80 border-t border-black/10 dark:border-white/10">
                    <span className="text-xs font-bold text-[#D946EF] mb-4 uppercase tracking-[0.2em]">{project.category}</span>
                    <h3 className="text-2xl font-bold text-foreground mb-4 group-hover:text-[#D946EF] transition-colors duration-300">{project.title}</h3>
                    <p className="text-textSecondary leading-relaxed mb-8 flex-1 line-clamp-3 font-medium">{project.description}</p>
                    <div className="flex flex-wrap gap-2 mt-auto">
                      {project.tags.map((tag, j) => (
                        <span key={j} className="text-xs font-bold text-textSecondary bg-black/5 dark:bg-white/5 px-3 py-1.5 rounded-full border border-black/10 dark:border-white/10">{tag}</span>
                      ))}
                    </div>
                  </div>
                </a>
          ...
          if (project.type === "overlay") {
            ...
            return (
              <div key={project.title} data-aos="fade-up" data-aos-delay={100 * i} className="md:col-span-1 h-full">
                <a href={project.link} className="group block h-full min-h-[400px] rounded-3xl overflow-hidden relative transition-all duration-500 hover:shadow-[0_0_40px_rgba(217,70,239,0.2)] border border-black/10 dark:border-white/10">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          if (project.type === "minimal") {
            ...
            return (
              <div key={project.title} data-aos="fade-up" data-aos-delay={100 * i} className="md:col-span-1 h-full">
                <a href={project.link} className="group flex flex-col h-full min-h-[400px] glass-panel rounded-3xl p-8 relative overflow-hidden transition-all duration-500 hover:border-[#D946EF]/30 bg-secondaryBg/50">
                  ...
                  <ExternalLink className="absolute top-8 right-8 w-5 h-5 text-textSecondary group-hover:text-foreground transition-colors duration-300" />

                  <span className="text-xs font-bold text-textSecondary uppercase tracking-[0.2em] mb-12">{project.category}</span>
                  <h3 className="text-2xl font-bold text-foreground mb-4 leading-tight group-hover:text-[#D946EF] transition-colors duration-300">{project.title}</h3>
                  <p className="text-textSecondary text-sm leading-relaxed mb-auto line-clamp-3 font-medium">{project.description}</p>

                  <div className="flex flex-wrap gap-2 mt-8">
                    {project.tags.map((tag, j) => (
                      <span key={j} className="text-xs font-bold text-textSecondary bg-black/5 dark:bg-white/5 px-3 py-1 rounded-full">{tag}</span>
                    ))}
                  </div>
                </a>
              </div>
            ...
            <div key={project.title} data-aos="fade-up" data-aos-delay={100 * i} className="md:col-span-1 h-full">
              <a href={project.link} className="group flex flex-col h-full glass-panel rounded-3xl overflow-hidden transition-all duration-500 hover:border-[#D946EF]/30 bg-secondaryBg/50">
                ...
                <div className="p-8 flex flex-col flex-1">
                  ...
                  <h3 className="text-xl font-bold text-foreground mb-3 group-hover:text-[#D946EF] transition-colors duration-300">{project.title}</h3>
                  <p className="text-textSecondary text-sm leading-relaxed mb-6 flex-1 line-clamp-3 font-medium">{project.description}</p>
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {project.tags.map((tag, j) => (
                      <span key={j} className="text-xs font-bold text-textSecondary bg-black/5 dark:bg-white/5 px-3 py-1 rounded-full border border-black/10 dark:border-white/10">{tag}</span>
                    ))}
                  </div>
                </div>
              </a>
